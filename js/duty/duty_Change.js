    ...
    mainPage = (function () {
        ...
        _m.table = function (filed) {
            ...
            $('#table1').bootstrapTable({
                url: urls + '/dutyImprove/getExchange.action',
                ...
                columns:
                    [{
                        field: 'form_teacher_account',
                        align: "center",
                        title: '调班教师',
                        formatter: function (value, row, index) {
                            var a= row.form_teacher_account.rname;
                            return a;
                        }
                    }, {
                        field: 'form_time',
                        ...
                        title: '调班日期',
                        formatter: function (value, row, index) {
                            var a= tool().getSmpFormatDateByLong(row.form_time,false);
                            return a;
                        }
                    }, {
                        field: 'to_teacher_account',
                        align: "center",
                        title: '目标教师',
                        formatter: function (value, row, index) {
                            var a= row.to_teacher_account.rname;
                            return a;
                        }
                    }, {
                        field: 'to_time',
                        ...
                        title: '目标日期',
                        formatter: function (value, row, index) {
                            var a= tool().getSmpFormatDateByLong(row.to_time,false);
                            return a;
                        }
                    }, {
                        field: '',
        ...
        _m.changeState = function (duty_id) {
            ...
            if(identity_id==3){
                var obj={exchange_id:duty_id}
                sl_Mask.duty_StateMask('是否同意本次调班申请',obj,function (params,inxex) {
        ...
        _m.changeStateAjax = function (params,index) {
            $.axse(urls+'/dutyImprove/exchangeExamine.action',params,function(result){
                if(params.agree==1){
                    sl_Mask.YesTip('审批成功');
                }else {
                    sl_Mask.YesTip('驳回成功');
