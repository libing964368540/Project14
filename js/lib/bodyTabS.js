...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ulHtml += '<a href="javascript:;"style="text-align:center;" class="chooseItem">';
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                         ulHtml += '<i class="seraph '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
                    }else{
                        // ulHtml += '<i class="layui-icon" data-icon="'+data[i].icon+'">'+data[i].icon+'</i>';
						if(data[i].title=='消息中心'){
                            ulHtml+='<span class="sidenav-link-icon">' +
                                '<i class="material-icons" style="font-size: 20px">'+data[i].icon+'</i>' +
                                '<em style="position: absolute;top: 17px;left: 12px;display: inline-block;width: 10px;height: 10px;background: #ff4747;border-radius: 50%;display: none;"id="MessageCountIcon"></em>'+
                                '</span>'
						}else {
                            ulHtml+='<span class="sidenav-link-icon">' +
                                '<i class="material-icons" style="font-size: 20px">'+data[i].icon+'</i>' +
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if(data[i].children[j].icon != undefined && data[i].children[j].icon != ''){
                        if(data[i].children[j].icon.indexOf("icon-") != -1){
                            ...
                            ulHtml+='<span class="sidenav-link-icon"><i class="material-icons" style="font-size: 20px">'+data[i].children[j].icon+'</i></span>'
                        }
                    }
                    ulHtml += '<cite>'+data[i].children[j].title+'</cite>'
						if(data[i].children[j].title=='我的消息'){
                            ulHtml +='<em style="position: absolute;padding: 0 5px;background: #ff4747;color: #ffffff;display: inline-block;height: 15px;line-height: 16px;font-size: 12px;right: 20px;top: 0;bottom: 0;margin:auto ;border-radius: 10px;text-align: center;font-style: normal;display: none;min-width: 20px;" id="MessageCount"></em>'
						}else {
                            ulHtml +='</a></dd>';
						}
                ...
                if(data[i].target == "_blank"){
                    ulHtml += '<a data-url="'+data[i].href+'" target="'+data[i].target+'" style="text-align:center">';
                }else{
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ...
                        // ulHtml += '<i class="layui-icon" data-icon="'+data[i].icon+'">'+data[i].icon+'</i>';
                        ulHtml+='<span class="sidenav-link-icon"><i class="material-icons" style="font-size: 20px;">'+data[i].icon+'</i></span>'
                    }
                }
		...
		if(_this.attr("target") == "_blank"){
			...
			if(_this.find("i.seraph,i.layui-icon").attr("data-icon") != undefined){
				if(_this.find("i.seraph").attr("data-icon") != undefined){
					...
					title += '<i class="material-icons">'+_this.find("i.layui-icon").attr("data-icon")+'</i>';
				}
			}
			//已打开的窗口中不存在
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
                ...
                sessionStorage.AgainLeave=_this.attr('data-id');
				tabIdIndex++;
				title += '<cite>'+_this.find("cite").text()+'</cite>';
				title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="'+tabIdIndex+'">&#x1006;</i>';
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='"+_this.attr("data-url")+"?v="+new Date().getTime()+"' data-id='"+tabIdIndex+"'></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
				...
				element.tabChange(tabFilter, that.getLayId(_this.find("cite").text()));
				that.tabMove(); //顶部窗口是否可滚动
                // if(_this.find("cite").text()!='退出'&&_this.find("cite").text().indexOf('发起')==-1){
                //     $(".clildFrame .layui-tab-item.layui-show").find("iframe")[0].contentWindow.location.reload();
                // }else {
                	if(_this.find("cite").text()=='发起'){
	...
	$(".refresh").on("click",function(){  //此处添加禁止连续点击刷新一是为了降低服务器压力，另外一个就是为了防止超快点击造成chrome本身的一些js文件的报错(不过貌似这个问题还是存在，不过概率小了很多)
		if($(this).hasClass("refreshThis")){
            ...
            top.layer.msg('<i class="material-icons msgIconStyleFail">cancel</i>'+'您点击的速度过快，还是等五秒刷新吧！',{
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!="后台首页"){
            ...
            top.layer.msg('<i class="material-icons msgIconStyleFail">cancel</i>'+'没有可以关闭的窗口了',{
                skin: 'msgStyle',//skin属性可以将layer的标签提取出来，重新定义样式
            });
		}
		//渲染顶部窗口
